...
public class ActionERTQPlanner extends APSRPlanner {
	...
	public ActionERTQPlanner(APSR psr, IQFunction qFunction1) 
	{
		this.psr = psr;
		this.data = null;
...
//					actionEnsembles = null;
	...
	private double[] Normalize_Distribution(double[] Z_Prime_Prob) throws Exception
	{
		...
		if (sum == 0)
		{
			throw new Exception("The sum of distribution is zero!");
		}
	...
	private HashMap<Action, ArrayList<Double>> computeTargets(List<PredictionVector> features, 
			...
			List<Double> rewards, List<Integer> testsLength, String name, int Pre_size)
			{
				...
				for(int i = 0; i < actions.size(); i++)
				{
					...
					{
						PredictionVector NextState = features.get(i + 1);
						maxNextState = -Double.MAX_VALUE;
						for(Action act : sampledData.getActionSet())
						{
							double value = actionEnsembles.get(act).getValueEstimate(NextState);
							maxNextState = Math.max(value, maxNextState);
						}
					}
					...
					inRunCount++;
					double var = rewards.get(i);
					double qvalue = var + aDiscount*maxNextState;
					qvalue = (double)(Math.round(qvalue * 1000000.0)) / 1000000.0;
	...
	private ArrayList<HashMap<Action, ArrayList<Double>>> computeTargets(List<PredictionVector> features, 
			...
			List<Double> rewards, List<Integer> testsLength, int Pre_size) throws Exception
			{
				...
				for(int i = 0; i < actions.size(); i++)
				{
					...
					double increment = (vmax - vmin) / Double.valueOf(num_atoms - 1);
					double[] updated_Prob;
					// maxNextState_Prob == null, it means the whole episode is only one action.
					if (maxNextState_Prob==null)
					{
						double[] Z_Prob = Generate_initial_Prob_distribution();
						double[] Z_Prime = Distributional_Belleman_Update(Z, rewards.get(i), aDiscount);
						updated_Prob = Projection_Between_Two_Distributions(Z_Prime, Z, Z_Prob, increment);
					}
					else
					{
						double[] update_dist = this.Distributional_Belleman_Update(Z, rewards.get(i), aDiscount);
						updated_Prob = this.Projection_Between_Two_Distributions(update_dist, Z, maxNextState_Prob, increment);
					}
					
					for (int idx = 0; idx < updated_Prob.length; idx++)
					{
						targets.get(idx).get(act1).add(updated_Prob[idx]);
					}
				}
				return targets;
			}
