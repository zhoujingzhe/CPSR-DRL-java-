...
package afest.datastructures.tree.decision.erts.informationfunctions.copy;
...
import afest.datastructures.tree.decision.erts.grower.SplitResult;
									   ...
									   implements IERTScore<R, C>
{
	...
	@Override
	public <T extends ITrainingPoint<R, C>> SplitResult<R, C, T> getScore(Collection<T> set, ISplit<R> split)
	{
		HashMap<Boolean, ArrayList<T>> splitSeparation = InformationFunctionsUtils.performSplit(set, split);
		SplitResult<R, C, T> ret = new  SplitResult<R, C, T>();
		...
		double score = (varS - trueSize/setSize * varSTrue - falseSize/setSize * varSFalse) / varS;
		ret.score = score;
		ret.LeftResult = splitSeparation.get(false);
		ret.RightResult = splitSeparation.get(true);
		return ret;
	}
